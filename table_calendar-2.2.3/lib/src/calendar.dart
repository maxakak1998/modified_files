...
class _TableCalendarState extends State<TableCalendar> with SingleTickerProviderStateMixin {
    ...
    return TableRow(
      children: widget.calendarController._visibleDays.value.take(7).map((date) {
        final weekdayString = widget.daysOfWeekStyle.dowTextBuilder != null
            ...
            : DateFormat.E(widget.locale).format(date).replaceAll('Mon', 'M').replaceAll('Tue', 'T')
            .replaceAll('Wed', 'W').replaceAll('Thu', 'T').replaceAll('Fri', 'F').replaceAll('Sat', 'S').replaceAll('Sun', 'S');;
  ...
  Widget _buildCellContent(DateTime date) {
    ...
    final tIsPreSelected = widget.calendarController.isPreSelectedDay(date);
    final tIsSelected = widget.calendarController.isSelected(date);
    final tIsSelectedArea = widget.calendarController.isBetweenSelectedArea(date);
    ...
    final tIsPreDay = widget.calendarController.compareDayMax(date);
    final tTwoDatePicked = widget.calendarController.check2DatePicked();
    ...
    if (isUnavailable) {
      ...
      return _CellWidget(
        ...
        isPreSelected: widget.enablePreDay ? tIsPreSelected : false,
        isSelectedArea: widget.enablePreDay ? tIsSelectedArea : false,
        isToday: tIsToday,
        isWeekend: tIsWeekend,
        isOutsideMonth: tIsOutside,
        isHoliday: tIsHoliday,
        calendarStyle: widget.calendarStyle,
        enablePre: widget.enablePreDay ? tTwoDatePicked : false,
        isPreDay: tIsPreDay,
      );
    }
  }
    ...
    if (widget.builders.singleMarkerBuilder != null) {
      ...
      return Container(
        ...
        decoration: BoxDecoration(
          ...
          color: event.mainEntityType == 'CANDIDATE' ? Colors.orange : Colors.blueAccent,
        ),
      );
    }
  }
